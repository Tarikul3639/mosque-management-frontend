...
export function ImageCard({
  ...
  return (
    <div className="group relative aspect-square overflow-hidden rounded-2xl border bg-muted/50 shadow-xs transition-all duration-300 hover:shadow-md">
      {/* Image */}
      <img
        src={url}
        alt=""
      ...
      {uploading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-background/60 backdrop-blur-xs">
          <ImageProgress
            progress={progress}
            size={80}
            strokeWidth={4}
      ...
      {removable && !uploading && (
        <div className="absolute inset-x-3 bottom-3 flex translate-y-2 items-center justify-between rounded-xl bg-background/80 p-2 shadow-lg backdrop-blur-md transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 sm:opacity-0">
          ...
          <Button
            ...
            type="button"
            variant="ghost"
            className="size-7 rounded-lg text-destructive transition-colors hover:bg-destructive hover:text-destructive-foreground max-sm:bg-destructive max-sm:text-destructive-foreground"
            onClick={() => onRemove?.(id)}
          >
